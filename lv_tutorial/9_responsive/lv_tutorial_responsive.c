...
void lv_tutorial_responsive(void)
{
    ...
    lv_style_copy(&style_new, &lv_style_plain);
    style_new.text.font = &lv_font_dejavu_10;               
    style_new.text.color = LV_COLOR_BLACK;
    lv_label_set_style(label10, &style_new);
    lv_label_set_text(label10, "Font 10");
    lv_obj_align(label10, btn3, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 20);

    lv_obj_t * label20 = lv_label_create(lv_scr_act(), NULL);
    static lv_style_t style_new20;                         /*Styles can't be local variables*/
    lv_style_copy(&style_new20, &lv_style_plain);
    style_new20.text.font = &lv_font_dejavu_20;               
    style_new20.text.color = LV_COLOR_BLACK;
    lv_label_set_style(label20, &style_new20);
    lv_label_set_text(label20, "Font 20");
    lv_obj_align(label20, label10, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 5);

    lv_obj_t * label30 = lv_label_create(lv_scr_act(), NULL);
    static lv_style_t style_new30;                         /*Styles can't be local variables*/
    lv_style_copy(&style_new30, &lv_style_plain);
    style_new30.text.font = &lv_font_dejavu_30;               
    style_new30.text.color = LV_COLOR_BLACK;
    lv_label_set_style(label30, &style_new30);
    lv_label_set_text(label30, "Font 30");
    lv_obj_align(label30, label20, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 5);

    lv_obj_t * label40 = lv_label_create(lv_scr_act(), NULL);
    static lv_style_t style_new40;                         /*Styles can't be local variables*/
    lv_style_copy(&style_new40, &lv_style_plain);
    style_new40.text.font = &lv_font_dejavu_40;               
    style_new40.text.color = LV_COLOR_BLACK;
    lv_label_set_style(label40, &style_new40);
    lv_label_set_text(label40, "Font 40");
    lv_obj_align(label40, label30, LV_ALIGN_OUT_BOTTOM_LEFT, 0, 5);
}
